...
class SiglipConfig: #Задаем параметры
    def __init__(
        self,
        hidden_size=768, 
...
class SiglipVisionEmbeddings(nn.Module):
    def __init__(self, config: SiglipConfig):
        ...
        #Конволюция возвращает тензор с 768 feature maps
        self.patch_embeddings = nn.Conv2d(config.num_channles, self.embed_dim, config.patch_size, config.patch_size, "valid")
    ...
    def forward(self, pixel_values: torch.FloatTensor):
        ...
        patch_embeds = self.patch_embeddings(pixel_values) #нарезаем картинки на патчи и возвращаем эмбеддинг каждого патча, и все это в одном большом тензоре 4D
...
class SiglipAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        #[Batch_size, Num_patches, Embed_dim] -> [Batch_size, Num_heads, Num_pathces, Head_dim]
        query_states = query_states.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1,2)
        key_states = key_states.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1,2)
        value_states = value_states.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1,2)
        #Считаем таблицу внимания
        ...
        # [Batch_size, Num_pathces, Num_heads, Head_dim] -> [Batch_size, Num_pathces, Embed_dim]
...
class SiglipVisionModel(nn.Module):
    ...
    def forward(self, pixel_values):
        #[Batch, Channel, Heidth, Width] -> [Batch, Num_patches, Embed_dim]
