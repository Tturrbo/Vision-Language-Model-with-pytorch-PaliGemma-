...
image_net_standart_mean = [0.5, 0.5, 0.5]
image_net_standart_std = [0.5, 0.5, 0.5]
...
class PaligemmaProcessor:
    ...
    def __init__(self, tokenizer, num_image_tokens:int, image_size:int):
        ...
        EXTRA_TOKENS = [f"<loc{i:04d}>" for i in range(1024)] #Детекция объектов на картинке
        EXTRA_TOKENS += [f"<seg{i:03d}>" for i in range(128)] #Сегментирование
    ...
    def __call__(self, text: List[str], images: List[Image.Image], padding: str="longest", truncation: bool=True):
        ...
        pixel_values = process_images(images, size=(self.image_size, self.image_size), resample=Image.Resampling.BICUBIC,
                                      rescale_factor = 1/255.0, image_mean=image_net_standart_mean, image_std=image_net_standart_std)
